...
- Авто-сжатие загружаемых референсов до 9MB, чтобы проходить серверные лимиты.
- Выбор соотношения сторон (Авто/1:1/16:9/9:16) перед генерацией в редакторе, генерации по образцу и примерке.
- GrsAI: fallback внутри провайдера — при 500/429/timeout повторяем запрос на `nano-banana-pro-cl`.
- Telegram-оповещения об ошибках backend/Celery (опционально, с лимитом по частоте).

### Changed
- SEO-генерация карточек примеров через `openai/gpt-4.1-mini` усилена против «бессмысленных» шаблонов: system prompt теперь жёстко требует предметные title и 2-3 конкретные детали сцены из prompt, а backend-нормализация добавляет quality guard и семантические fallback-описания (одежда/фон/свет/ракурс) вместо абстрактных фраз.
...
- Качество SEO-автозаполнения улучшено для сложных EN-prompts: добавлены контекстные RU-хайлайты сцены (style/action/composition), усилены анти-шаблонные правила для title/description; для OpenRouter теперь используется только одна SEO-модель `openai/gpt-4.1-mini` (без второго запроса к `gpt-4o-mini`).
- OAuth callback-страницы (`/vk/callback`, `/yandex/callback`) теперь сохраняют и применяют `next`-маршрут после входа, чтобы пользователь возвращался на исходный экран (например, генерацию по выбранному примеру), а не на главную.
...
- Выполнена сквозная русификация пользовательских сообщений: обновлены тексты форм входа/регистрации/сброса пароля, профиля, админ-панели и сопутствующих экранов.
- Текст welcome-оповещения теперь использует кликабельный формат ссылки: слово "Telegram-канал" открывает канал напрямую из раздела "Оповещения".
- Email-регистрация упрощена: поля "Имя" и "Фамилия" удалены из формы и из backend-схемы запроса регистрации.
- Таймаут ожидания генерации для GrsAI/kie.ai увеличен до 5 минут.
- Лимит запросов статуса/результата генерации увеличен, 429 теперь не рвут ожидание на фронте.
- На экранах редактирования и примерки лимит исходного изображения для клиентской обработки увеличен до 40MB (с автосжатием до 9MB перед отправкой).
...
### Fixed
- В WAF-конфиге добавлена прокси-маршрутизация `location /e/` в backend, чтобы короткие публичные ссылки `/e/<id>` корректно работали на прод-домене (а не отдавали frontend SPA как `200`).
...
- Ошибки генерации больше не показывают технические детали пользователю; при неуспехе ⭐️ не списываются.
- Ожидание результата генерации на фронтенде адаптировано до 10 минут: polling больше не обрывается на ~5 минуте во время fallback `nano-banana-pro -> nano-banana-pro-cl`, добавлены дополнительные UX-предупреждения о долгой генерации.
