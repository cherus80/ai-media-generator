...
### Changed
- На формах входа и регистрации скрыт Google Sign-In; оставлены VK/Yandex и email-пароль, а глобальная загрузка Google Identity Services из HTML убрана.
- Регистрация по email упрощена: форма и backend больше не требуют “сильный” пароль с обязательными спецсимволами, цифрами и разным регистром; для регистрации достаточно любого непустого пароля, а индикатор “надежности” убран с экрана регистрации.
...
- Ускорен публичный каталог примеров: `GET /api/v1/content/examples` получил `in-process` cache (`Cache-Control: public, max-age=90`), безопасный `view=card` DTO с пагинацией `page/page_size`, а frontend переведён на lazy route chunks (`auth/public/app/admin`) и lightweight thumbnails с skeleton/retry UX для `/app/examples`.
...
### Changed
- Метрики дашборда админки теперь считаются только по обычным пользователям (`role=USER`): действия `ADMIN/SUPER_ADMIN` исключены из пользовательских/генерационных/подписочных и платёжных агрегатов.
- Трекинг карточек примеров обновлён: события `view/start` теперь сохраняют `actor_user_id`, а действия `ADMIN/SUPER_ADMIN` не увеличивают `uses_count` и не пишутся в A/B-метрики.
...
### Fixed
- Для production web auth добавлен внешний заголовок `Cross-Origin-Opener-Policy: same-origin-allow-popups` в `Caddyfile`, чтобы popup-flow Google Sign-In корректно отрабатывал на мобильных браузерах и не зависал на пустом белом окне до отправки `id_token` на backend.
- Исправлен production redirect для prerendered SEO-маршрутов SPA: frontend nginx теперь сначала ищет `/$uri/index.html`, поэтому bare routes (`/login`, `/register`, `/forgot-password`, `/app*`, `/pricing`, `/privacy`, `/contacts`) отдают route-specific raw HTML без `301` на trailing slash и без потери server-injected `<head>`.
- Улучшен fallback для SEO-генерации по структурированным и смешанным промптам (JSON/текст): эвристики теперь выделяют смысловые поля сцены/света/камеры/гардероба и игнорируют технические блоки (`identity_lock`, `avoid`, render-метаданные), чтобы не получались общие описания вроде «сохранение черт лица».
