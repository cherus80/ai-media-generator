...
**Использование:**
1) Откройте `backup-from-vps.sh` в корне проекта и при необходимости поправьте `VPS_USER`, `VPS_HOST`, `VPS_PROJECT_DIR` (по умолчанию подставлены из deploy-скриптов).
2) Запустите:
```bash
./backup-from-vps.sh
```
